...
namespace NBitcoin
{
    public partial class Network
    {
        ...
        public static Network ImpleumMain => Network.GetNetwork(nameof(ImpleumMain)) ?? InitImpleumMain();

        public static Network ImpleumTest => Network.GetNetwork(nameof(ImpleumTest)) ?? InitImpleumTest();

        public static Network ImpleumRegTest => Network.GetNetwork(nameof(ImpleumRegTest)) ?? InitImpleumRegTest();
        ...
        private static Network InitImpleumTest()
        {
            ...
            var consensus = Network.ImpleumMain.Consensus.Clone();
            consensus.PowLimit = new Target(uint256.Parse("0000ffff00000000000000000000000000000000000000000000000000000000"));
            ...
            genesis.Header.Time = 152336701;
            genesis.Header.Nonce = 1503601;
            genesis.Header.Bits = consensus.PowLimit;
            consensus.HashGenesisBlock = genesis.GetHash(consensus.NetworkOptions);

            Assert(consensus.HashGenesisBlock == uint256.Parse("dab06c78af72a1fb8a17581c631ec2d3df60a8029142962de5448b992b47467e"));
            ...
            var builder = new NetworkBuilder()
                ...
                .AddDNSSeeds(new[]
                {
                    new DNSSeedData("94.131.240.45", "94.131.240.45")
                });

            builder.AddSeeds(new[]
            {
                new NetworkAddress(IPAddress.Parse("94.131.240.45"), builder.Port),
                new NetworkAddress(IPAddress.Parse("62.80.181.141"), builder.Port)
            }); // the c# testnet node
